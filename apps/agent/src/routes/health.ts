...
healthRouter.get('/', (c) => c.json({
  ...
  providers: { voice: providers.voice(), call_channel: callChannel(), email: providers.email(), llm: config.geminiApiKey ? 'gemini' : 'missing' },
  configured: {
    ...
    elevenlabs_whatsapp_phone_number_id: !!config.elevenlabs.whatsappPhoneNumberId,
    whatsapp_call_permission_template: !!config.elevenlabs.whatsappCallPermissionTemplate,
